<html xmlns:text-algin="http://www.w3.org/1999/xhtml" xmlns:text-align="http://www.w3.org/1999/xhtml">
<head>
<title>
    DIP
</title>
    <meta name="viewport" content="width=device-width, initial-scale=1">
    <link rel="stylesheet" href="https://maxcdn.bootstrapcdn.com/bootstrap/3.3.7/css/bootstrap.min.css">
    <script src="https://ajax.googleapis.com/ajax/libs/jquery/3.3.1/jquery.min.js"></script>
    <script src="https://maxcdn.bootstrapcdn.com/bootstrap/3.3.7/js/bootstrap.min.js"></script>
</head>
<body>

    <div class="container">
        <div class="panel panel-primary">
            <div class="panel-heading"><center>Personal Details</center></div>
            <div class="panel-body">
                <div class="fluid-container">
                    <div class="row">
                        <div class="col-md-4">
                            <div class="form-group">
                                <label for="usr">Name:</label>
                                <input type="text" class="form-control" id="name" name="name">
                            </div>
                        </div>
                        <div class="col-md-4">
                            <div class="form-group">
                                <label for="usr">Email-Id</label>
                                <input type="email" class="form-control" id="email" name="email">
                            </div>
                        </div>

                        <div class="col-md-4">
                            <div class="form-group">
                                <label for="usr">Phone:</label>
                                <input type="text" class="form-control" id="phone" name="phone">
                            </div>
                        </div>
                    </div>
                    <div class="row">
                        <div class="col-md-4">
                            <div class="form-group">
                                <label for="usr">Date Of Birth:</label>
                                <input type="date" class="form-control" id="dob" name="dob">
                            </div>
                        </div>
                        <div class="col-md-4">
                            <div class="form-group">
                                <label for="usr">Gender:</label>
                                <br>
                                <label class="radio-inline">
                                    <input type="radio" name="optradio">Male
                                </label>
                                <label class="radio-inline">
                                    <input type="radio" name="optradio">Female
                                </label>
                            </div>
                        </div>
                        <div class="col-md-4">
                            <div class="form-group">
                                <label for="usr">Place of Birth:</label>
                                <input type="text" class="form-control" id="pob" name="pob">
                            </div>
                        </div>
                    </div>

                    <div class="row">
                        <div class="col-md-4">
                            <div class="form-group">
                                <label for="usr">Caste:</label>
                                <input type="text" class="form-control" id="caste" name="caste">
                            </div>
                        </div>
                        <div class="col-md-4">
                            <div class="form-group">
                                <label for="usr">Sub-Caste:</label>
                                <input type="text" class="form-control" id="subcaste" name="subcaste">
                            </div>
                        </div>
                        <div class="col-md-4">
                            <div class="form-group">
                                <label for="usr">Marital Status:</label>
                                <input type="text" class="form-control" id="status" name="status">
                            </div>
                        </div>
                    </div>
                    <hr style="height:1px;border:none;color:#333;background-color: blue;">
                    <h5>Parents Details</h5>
                    <div class="row">
                        <div class="col-md-4">
                            <div class="form-group">
                                <label for="usr">Father's Name:</label>
                                <input type="text" class="form-control" id="caste" name="caste">
                            </div>
                        </div>
                        <div class="col-md-4">
                            <div class="form-group">
                                <label for="usr"> Father's Occupation:</label>
                                <input type="text" class="form-control" id="subcaste" name="subcaste">
                            </div>
                        </div>
                        <div class="col-md-4">
                            <div class="form-group">
                                <label for="usr">Father's Salary:</label>
                                <input type="text" class="form-control" id="status" name="status">
                            </div>
                        </div>
                    </div>
                    <div class="row">
                        <div class="col-md-4">
                            <div class="form-group">
                                <label for="usr">Mother's Name</label>
                                <input type="text" class="form-control" id="caste" name="caste">
                            </div>
                        </div>
                        <div class="col-md-4">
                            <div class="form-group">
                                <label for="usr">Mother's Occupation</label>
                                <input type="text" class="form-control" id="subcaste" name="subcaste">
                            </div>
                        </div>
                        <div class="col-md-4">
                            <div class="form-group">
                                <label for="usr">Mother's Salary</label>
                                <input type="text" class="form-control" id="status" name="status">
                            </div>
                        </div>
                    </div>
                    <hr style="height:1px;border:none;color:#333;background-color: blue;">
                    <h5>Residential Address</h5>
                    <div class="row">
                        <div class="col-md-4">
                            <div class="form-group">
                                <label for="usr">Permanent Address:</label>
                                <input type="textarea" class="form-control" id="caste" name="caste">
                            </div>
                        </div>
                        <div class="col-md-4">
                            <div class="form-group">
                                <label for="usr">State:</label>
                                <input type="text" class="form-control" id="subcaste" name="subcaste">
                            </div>
                        </div>
                        <div class="col-md-4">
                            <div class="form-group">
                                <label for="usr">Country:</label>
                                <input type="text" class="form-control" id="status" name="status">
                            </div>
                        </div>
                    </div>
                    <hr style="height:1px;border:none;color:#333;background-color: blue;">
                    <h5>Income Details</h5>
                    <div class="row">
                        <div class="col-md-4">
                            <div class="form-group">
                                <label for="usr">Income Certificate Number:</label>
                                <input type="text" class="form-control" id="caste" name="caste">
                            </div>
                        </div>
                        <div class="col-md-4">
                            <div class="form-group">
                                <label for="usr">Self Income:</label>
                                <input type="text" class="form-control" id="subcaste" name="subcaste">
                            </div>
                        </div>
                        <div class="col-md-4">
                            <div class="form-group">
                                <label for="usr">Family Income:</label>
                                <input type="text" class="form-control" id="status" name="status">
                            </div>
                        </div>
                    </div>

                    <div class="row">
                        <div class="col-md-4">
                            <div class="form-group">
                                <label for="usr">Date of Issue:</label>
                                <input type="text" class="form-control" id="caste" name="caste">
                            </div>
                        </div>
                        <div class="col-md-4">
                            <div class="form-group">
                                <label for="usr">Issuing Authority</label>
                                <input type="text" class="form-control" id="subcaste" name="subcaste">
                            </div>
                        </div>
                    </div>
                    <hr style="height:1px;border:none;color:#333;background-color: blue;">
                    <h5>Caste Details</h5>
                    <div class="row">
                        <div class="col-md-4">
                            <div class="form-group">
                                <label for="usr">Caste:</label>
                                <input type="text" class="form-control" id="caste" name="caste">
                            </div>
                        </div>
                        <div class="col-md-4">
                            <div class="form-group">
                                <label for="usr">Caste Certificate Number:</label>
                                <input type="text" class="form-control" id="subcaste" name="subcaste">
                            </div>
                        </div>
                        <div class="col-md-4">
                            <div class="form-group">
                                <label for="usr">Issuing Date:</label>
                                <input type="text" class="form-control" id="status" name="status">
                            </div>
                        </div>
                    </div>
                    <div class="row">
                        <center> <input type="submit" class="btn btn-primary" value="Submit"></center>
                    </div>
                </div>
            </div>
        </div>
    </div>
</body>
</html>